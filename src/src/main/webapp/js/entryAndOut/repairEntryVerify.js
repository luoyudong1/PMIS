    ...
    function($, bootstrap, dataTable, CMethod) {
        ...
        function(e) { // 获取当前单号
            if (sheetTable.data().any()) {
                ...
                sourceStoreHouseId=$('#supplierNameAdd:contains("'+sheetTrData.sourceStorehouseName+'")').val()
                var params = $.param({
                    sheetId: sheet_id2
                });
        ...
        function(e) {
            ...
            $.ajax({
                url: config.basePath + '/entryAndOut/repairEntry/modify',
                type: 'POST',
                data: params,
                contentType: 'application/json',
                dataType: "json",
            ...
            $.ajax({
                ...
                success: function(result) {
                    if (result.code != 0) {
                        ...
                        sheetTable.ajax.reload();
                        $("#alertMsg").html('<span style="color:green;text-align:center"><strong>单据审核完成！</strong></span>');
                        $("#infoAlert").show();
                        hideTimeout("infoAlert", 2000);
                    }
                }
