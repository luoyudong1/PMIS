    ...
    function (config) {
        ...
        require(['jquery', 'bootstrap', 'common/dt', 'common/commonMethod', 'common/slider', 'common/dateFormat', 'common/select2', 'common/pinyin'],
            function ($, bootstrap, dataTable, CMethod) {
                ...
                $.ajax({
                    async: false,
                    url: config.basePath + "/entryAndOut/repairEntry/getAllReceiptStoreHouse",
                    data: {
                        "action": "all"
                    },
                    dataType: 'json',
                ...
                var sheetTable = dataTable('sheetTable', {
                    bAutoWidth: false,
                    ajax: {
                        url: config.basePath + '/entryAndOut/repairEntry/getAllSheets',
                        type: 'GET',
                        data: function (d) {
                ...
                var tblPartsInfo = dataTable('tblPartsInfo', {
                    bAutoWidth: false,
                    ajax: {
                        url: config.basePath + '/entryAndOut/repairEntry/getAllParts',
                        type: 'get',
                        data: function (d) {
                ...
                $('#sheetTable tbody').on('click', 'tr',
                    function (e) { // 获取当前单号
                        if (sheetTable.data().any()) {
                            ...
                            tblPartsInfo.ajax.reload()
                            sheetDetailTable.column(0).search('');
                            sheetDetailTable.ajax.url(config.basePath + '/entryAndOut/repairEntry/getAllSheetDetails?' + params).load();
                        }
                    });
                ...
                var sheetDetailTable = dataTable('sheetDetailTable', {
                    ...
                    ajax: {
                        type: 'GET',
                        data: "",
                        url: config.basePath + '/entryAndOut/repairEntry/getAllSheetDetails'
                    },
                    columns: [{
                        data: null
                    },
                ...
                $('#addSheetModal').on('show.bs.modal',
                    function () {
                        ...
                        // 单据ID为单据类型+部门id+序列号
                        if (deptId < 10) {
                            var sheetId = "10-0" + deptId + "-";
                        } else {
                            var sheetId = "10-" + deptId + "-";
                        }
                        $.ajax({
                ...
                $("#btnAddSheetOk").on('click',
                    function (e) {
                        ...
                        $.ajax({
                            url: config.basePath + '/entryAndOut/repairEntry/createSheetInfo',
                            type: "post",
                            data: params,
                            contentType: 'application/json',
                            dataType: "json",
                ...
                function verifyNewPartCode() {
                    var verifyNewPartCodeFlag=0;
                    $.ajax({
                        async: false,
                        url: config.basePath + '/entryAndOut/repairEntry/verifyNewPartCode',
                        type: "get",
                        data: {
                            newPartCode: $('#part_codeNewAdd').val(),
                        },
                        contentType: 'application/json',
                        success: function (result) {
                            if (result) {
                                verifyNewPartCodeFlag=1

                            } else {
                                $("#alertMsgAdd").html("<font style='color:red'>新出厂编码已存在，请检查并重新输入</font>");
                                $("#alertMsgAdd").css('display', 'inline-block')
                                CMethod.hideTimeout("alertMsgAdd", "alertMsgAdd", 5000);
                            }
                        }
                    })
                    if(verifyNewPartCodeFlag==0){
                        return false;
                    }
                    return true;
                }
                ...
                $("#btnAddSheetDetailOk").on('click',
                    function (e) {
                        ...
                        if (verifyPartsAdd() == false) {
                            return false;
                        }else if (verifyNewPartCode() == false) {
                            alert(verifyNewPartCode())
                            return false;
                        }else {
                            var params = JSON.stringify({
                            ...
                            $.ajax({
                                url: config.basePath + '/entryAndOut/repairEntry/SheetDetailCreate',
                                type: "post",
                                data: params,
                                contentType: 'application/json',
                                dataType: "json",
                                success: function (result) {
                                    if (result.code != 0) {
                                        ...
                                        tblPartsInfo.ajax.reload()
                                        sheetDetailTable.ajax.url(config.basePath + '/entryAndOut/repairEntry/getAllSheetDetails?' + json).load();
                ...
                $("#btnModifySheetDetailOk").on('click',
                    function (e) {
                        ...
                        var params = JSON.stringify({
                            ...
                            newPartCode: $('#part_codeNewModify').val(),
                        ...
                        $.ajax({
                            url: config.basePath + '/entryAndOut/repairEntry/SheetDetailModify',
                            type: "post",
                            data: params,
                            contentType: 'application/json',
                            dataType: "json",
                ...
                $("#btnModifySheetOk").on('click',
                    function (e) {
                        ...
                        $.ajax({
                            url: config.basePath + '/entryAndOut/repairEntry/modifyVerify',
                            type: 'POST',
                            data: params,
                            contentType: 'application/json',
                            dataType: "json",
                ...
                $('#btnPopSheetOk').on('click',
                    function (e) {
                        ...
                        $.ajax({
                            url: config.basePath + '/entryAndOut/repairEntry/delete',
                            type: 'POST',
                            data: params,
                            contentType: 'application/json',
                            dataType: 'json',
                ...
                $('#btnPopOk').on('click',
                    function (e) {
                        ...
                        $.ajax({
                            url: config.basePath + '/entryAndOut/repairEntry/SheetDetailDeleteByCode',
                            type: 'POST',
                            data: params,
                            contentType: 'application/json',
                            dataType: 'json',
